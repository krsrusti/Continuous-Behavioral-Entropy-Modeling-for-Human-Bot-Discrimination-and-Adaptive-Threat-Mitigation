...
def risk():
    ...
    if model is None:
        return jsonify({
            'error': 'Model not trained yet. Run room4_classifier/train.py first.'
        }), 503
    ...
    # ── Prediction ────────────────────────────────────────────────────
    # The trained model returns string labels:
    # 'human', 'bot', or 'llm'
    prediction = str(pred)

    # Use the actual class order stored by the Random Forest.
    # This avoids assuming that human=0, bot=1, llm=2.
    probabilities = dict(
        zip(model.classes_.tolist(), proba)
    )

    # Make sure all expected classes exist in the response.
    probabilities = {
        'human': float(probabilities.get('human', 0.0)),
        'bot': float(probabilities.get('bot', 0.0)),
        'llm': float(probabilities.get('llm', 0.0))
    }
